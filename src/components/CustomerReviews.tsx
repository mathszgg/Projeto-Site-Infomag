    ...
    return (
        <div className="bg-secondary-color text-white flex flex-col items-center gap-10">
            <div className="mt-0 flex flex-col justify-center items-center gap-4">
                <ScrollAnimation direction="left"><h2 className="text-gray-900 text-5xl font-bold">Avaliações dos Clientes</h2></ScrollAnimation>
                <ScrollAnimation direction="right"><StarsRating count={5} w={40} h={40} /></ScrollAnimation>
            </div>
                <ScrollAnimation className="flex flex-row justify-center gap-10" direction="bottom">
                {reviews.map((item, index) => (
                    <div key={index} className="transition-all bg-primary-color shadow-lg flex flex-col w-sm rounded-lg p-5 hover:scale-105 hover:bg-green-700 hover:shadow-xl shadow-gray-400">
